...
export default function PrInput({ data, onChange }: Props) {
  ...
  const handleRemove = (i: number) => {
    ...
    setSeries(next)
    const newPrecision = next.map(s => parseCol(s.precisionText))
    const newRecall = next.map(s => parseCol(s.recallText))
    const newAp = next.map(s => parseFloat(s.ap) || 0)
  ...
  const handleSample = () => {
    setSeries(SAMPLE_LABELS.map((label, i) => ({
      label,
      ap: String(SAMPLE.ap[i]),
      precisionText: SAMPLE.precision[i].join('\n'),
        ...
        <div key={i} style={{ border: '1px solid #E5E7EB', borderRadius: 8, padding: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <input
              type="number"
              value={s.ap}
              onChange={e => updateSeries(i, { ap: e.target.value })}
              step={0.001}
              min={0}
              max={1}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <textarea
                rows={5}
                value={s.recallText}
                onChange={e => updateSeries(i, { recallText: e.target.value })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <textarea
                rows={5}
                value={s.precisionText}
                onChange={e => updateSeries(i, { precisionText: e.target.value })}
                style={{ ...inputStyle, padding: '4px 8px', resize: 'vertical', width: '100%', boxSizing: 'border-box' }}
              />
            </div>
          </div>
