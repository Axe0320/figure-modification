...
interface Props {
  ...
  onChange: (patch: Partial<LinePlotParams>) => void
  onReset: () => void
}
...
export default function LinePlotEditor({ figure, onChange, onReset }: Props) {
  const [open, setOpen] = useState<Section>('text')
  const p = figure.params

  const nSeries = Array.isArray(figure.data.y[0])
    ? (figure.data.y as number[][]).length : 1
            ...
            <div className="px-3 pb-3 pt-1 space-y-3" style={{ borderTop: '1px solid #EDE9FE' }}>
              ...
              {key === 'axis' && (
                <>
                  ...
                  <Toggle label="X軸 対数スケール" value={p.log_scale_x} onChange={(v) => onChange({ log_scale_x: v })} />
                  <Toggle label="Y軸 対数スケール" value={p.log_scale_y} onChange={(v) => onChange({ log_scale_y: v })} />
                </>
              )}
