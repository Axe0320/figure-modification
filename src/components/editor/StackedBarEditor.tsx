...
import type { StackedBarState, StackedBarParams } from '../../types/figures'
...
interface Props {
  figure: StackedBarState
  onChange: (patch: Partial<StackedBarParams>) => void
  onReset: () => void
}
...
export default function StackedBarEditor({ figure, onChange, onReset }: Props) {
  const [open, setOpen] = useState<Section>('text')
  const p = figure.params
  const nSeries = figure.data.values.length
            ...
            <div className="px-3 pb-3 pt-1 space-y-3" style={{ borderTop: '1px solid #EDE9FE' }}>
              ...
              {key === 'display' && (
                <>
                  ...
                  <Toggle label="100%積み上げ（正規化）" value={p.normalize} onChange={(v) => onChange({ normalize: v })} />
