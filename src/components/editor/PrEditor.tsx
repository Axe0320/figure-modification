...
import type { PrState, PrParams } from '../../types/figures'
...
interface Props {
  figure: PrState
  onChange: (patch: Partial<PrParams>) => void
  onReset: () => void
}
...
export default function PrEditor({ figure, onChange, onReset }: Props) {
  const [open, setOpen] = useState<Section>('text')
  const p = figure.params
  const nSeries = figure.data.precision.length
            ...
            <div className="px-3 pb-3 pt-1 space-y-3" style={{ borderTop: '1px solid #EDE9FE' }}>
              {key === 'text' && (
                <>
                  ...
                  <div><label className="block text-xs text-gray-500 mb-1">X軸ラベル</label>
                    <ImeInput value={p.xlabel} onValueChange={(v) => onChange({ xlabel: v })} placeholder="Recall" className="w-full text-sm px-3 py-1.5" /></div>
                  <div><label className="block text-xs text-gray-500 mb-1">Y軸ラベル</label>
                    <ImeInput value={p.ylabel} onValueChange={(v) => onChange({ ylabel: v })} placeholder="Precision" className="w-full text-sm px-3 py-1.5" /></div>
              ...
              {key === 'display' && (
                <>
                    ...
                    <div className="space-y-2">
                      {Array.from({ length: nSeries }, (_, i) => (
                        <div key={i} className="space-y-1">
                          <span className="text-xs text-gray-400 block">{p.legend[i] ?? `Class ${i}`}</span>
                  ...
                  <Toggle label="AP値を凡例に表示" value={p.show_ap_in_legend} onChange={(v) => onChange({ show_ap_in_legend: v })} />
                  <div>
