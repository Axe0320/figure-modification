...
interface Props {
  ...
  onChange: (patch: Partial<HistogramParams>) => void
  onReset: () => void
}
...
export default function HistogramEditor({ figure, onChange, onReset }: Props) {
              ...
              {key === 'text' && (
                <>
                  ...
                  <div><label className="block text-xs text-gray-500 mb-1">Y軸ラベル</label>
                    <ImeInput value={p.ylabel} onValueChange={(v) => onChange({ ylabel: v })} placeholder="（自動）" className="w-full text-sm px-3 py-1.5" /></div>
