...
interface Props {
  ...
  onChange: (patch: Partial<BarChartParams>) => void
  onReset: () => void
}
...
function LimInput({ label, value, onChange }: {
  label: string; value: [number, number] | null
...
function StepInput({ label, value, onChange }: {
...
export default function BarChartEditor({ figure, onChange, onReset }: Props) {
  const [open, setOpen] = useState<Section>('text')
  const p = figure.params

  // 系列数を data から計算
  const nSeries = Array.isArray(figure.data.values[0])
    ? (figure.data.values as number[][]).length : 1

  const setColor = (i: number, color: string) => {
    const next = [...p.colors]
    while (next.length <= i) next.push('#6C63FF')
    next[i] = color
                ...
                <>
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {([['vertical', '縦'], ['horizontal', '横']] as const).map(([val, lbl]) => (
                        <button key={val} onClick={() => onChange({ orientation: val })} style={is(p.orientation === val)}>{lbl}</button>
                      ))}
                    </div>
                  </div>
                  <Slider label="バー幅" value={p.bar_width} min={0.1} max={1} step={0.05} onChange={(v) => onChange({ bar_width: v })} />
